...
struct WasmEdge::AOT::Compiler::CompileContext {
  ...
  std::vector<
      std::tuple<uint32_t, llvm::Function *, llvm::DIScope *,
                 const WasmEdge::AST::CodeSegment *, const llvm::DWARFDie *>>
      Functions;
...
public:
  FunctionCompiler(AOT::Compiler::CompileContext &Context, llvm::DIBuilder *DIB,
                   llvm::Function *F, llvm::DIScope *DIScope,
                   const llvm::DWARFDie *D, Span<const ValType> Locals,
                   uint32_t CodeOffset, bool InstructionCounting,
                   bool GasMeasuring, bool OptNone)
      : Context(Context), LLContext(Context.LLContext), OptNone(OptNone), F(F),
        DWARFDie(D), Builder(llvm::BasicBlock::Create(LLContext, "entry", F)),
        DIBuilder(DIB), CodeOffset(CodeOffset) {
    if (F) {
    ...
    auto Dispatch = [this](const AST::Instruction &Instr) -> void {
      ...
      if (LineTable) {
        ...
        if (LineTable->getFileLineInfoForAddress(
                ...
                Result)) {
          //auto *DIScope = Context.getOrCreateFile(*DIBuilder, Result.FileName);
      ...
      if (auto Iter = DIVars.find(Offset); Iter != DIVars.end()) {
        for (auto [DIVar, L, I] : Iter->second) {
          ...
          case 1:
          case 3:
            Storage = Context.getGlobals(Builder, ExecCtx, I);
            break;
          case 2:
            if (Stack.size() > I) {
              Value = Stack[I];
...
void Compiler::compile(const AST::FunctionSection &FuncSec,
  ...
  for (size_t I = 0; I < TypeIdxs.size() && I < CodeSegs.size(); ++I) {
    ...
    Context->Functions.emplace_back(TypeIdx, F, DIScope, &Code, DWARFDie);
  ...
  for (auto [T, F, S, Code, D] : Context->Functions) {
    if (!Code) {
      continue;
    }
    ...
    FunctionCompiler FC(*Context, &DIBuilder, F, S, D, Locals,
                        CodeSec.getOffset(), InstructionCounting, GasMeasuring,
                        optNone());
